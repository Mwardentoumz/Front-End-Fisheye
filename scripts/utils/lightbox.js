class Lightbox {
  constructor (media, photographer, portfolio) {
    ...
    _next (e) {
      e.preventDefault()
      console.log(this._portfolio.findIndex(media => media._media.id == this._media.id))
      let i = this._portfolio.findIndex((media => media._media.id == this._media.id))
      
      
  
      this._media = this._portfolio[i + 1]._media
      this._template.loadFactory(this._media)
      this._initControls()
    }
